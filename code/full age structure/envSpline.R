...
f <- function(parms,dat)
 {
  ...
  llmld <- llsst <- 0
  sigma_spline_mld <- exp(logsigma_spline_mld)
  sigma_spline_sst <- exp(logsigma_spline_sst)
  lambda_spline_mld <- exp(loglambda_spline_mld)
  lambda_spline_sst <- exp(loglambda_spline_sst)
  llenv <- rep(0,Nfeed) # likelihood
  penv <- rep(0,Nfeed) # penalty
  
  for(i in 1:Nfeed){
    llmld <- -sum(dnorm(beta_splines_mld[i],mean=0,sd=sigma_spline_mld[i],log=T))
    llsst <- -sum(dnorm(beta_splines_sst[i],mean=0,sd=sigma_spline_sst[i],log=T))
    llenv[i] <- llmld+llsst
  }
  for(i in 1:Nfeed){
    mldS <- Smld[,,i]
  ...
  datalike <- LogLike1 + sum(LogLike2a) + sum(LogLike2b)+sum(llenv);
