...
f <- function(parms,dat)
 {
  ...
  # Fecundity (by breeding ground) at carrying capacity
  ParA <- (fmax-f0)/f0
  # (unsure if this is the way)
  # ParA <- ParA*Mix

  # Annual feeding ground survival deviations
  SFdevYr <- matrix(0,nrow=Nfeed,ncol=Nyr)
  YrMatch=Nyr-(Yr2-YrSDevs)
  whichFeed <- which(SF==1)
  SFdevYr[whichFeed, YrMatch:Nyr] <- matrix(SFdev, nrow = length(whichFeed))
    ...
    for (Ibreed in 1:Nbreed){
      for (Ifeed in 1:Nfeed){
        if (MixI[Ibreed,Ifeed] != 0){
          ...
          SurvOutF[Ifeed,Year] <- 1.0/(1.0+exp(LogitSA+SFdevYr[Ifeed,Year]));
  ...
  # Survival devs as random effects
  ...
  Penal <- 0;
  # penalty on fmax
  Penal_fmax <- 100.0/(1+exp(-1000.0*(fmax-0.98))) |> sum()
  Penal <- Penal_fmax
  # overall K
  TotalK <- sum(BreedK)
