...
plot_survival_curve <- function(obj,opt="F"){
  ...
  psst <- sdat |> 
    ...
    # facet_wrap(~zone)
  pmld <- sdat |> 
    ggplot(aes(mld,survival))+
    geom_point(aes(color=zone))+
    # geom_text(aes(label=year,color=year),size=2)+
    # geom_arrow_chain(color='gray80',linewidth=0.25,arrow_head="head_wings")+
    labs(x="Mixed Layer Depth",y="Survival")+
    scale_color_manual(values=viridis_pal()(nzone))
    # facet_wrap(~zone)
  cowplot::plot_grid(psst,pmld,nrow=2)
}
...
plot_Sdevs_curve <- function(obj){
  ...
  pmld <- sdat |> 
    ...
    # geom_text(aes(label=year,color=year),size=2)+
    # geom_arrow_chain(color='gray80',linewidth=0.25,arrow_head="head_wings")+
    labs(x="Mixed Layer Depth",y="Survival Deviate")+
    scale_color_manual(values=viridis_pal()(nzone))
  # facet_wrap(~zone)
  cowplot::plot_grid(psst,pmld,nrow=2)
}
...
plot_omegas <- function(obj){
  ...
  dat <- pluck(obj,"sdfixed") |> as_tibble(rownames="param") |> 
    filter(grepl('omega',param)) |>
    dplyr::select(1:3) |> 
    set_names(c("param","mean","se")) |> 
    mutate(lower=mean-1.96*se,upper=mean+1.96*se) |> 
    mutate(zone=rep(zn,2))
...
plot_varK <- function(obj){
  ...
  kdat <- kdat |> 
    ...
    mutate(upper.K=Kyr+1.96*sd.Kyr,lower.K=Kyr-1.96*sd.Kyr) |> 
    mutate(lower.K=pmax(lower.K,0))
  
  p <- kdat |> 
    ggplot(aes(year,log(Kyr),ymin=log(lower.K),ymax=log(upper.K)))+
    geom_ribbon(fill="lightblue")+
    geom_line()+
    labs(x="Year",y="K")+
    facet_wrap(~zone)
