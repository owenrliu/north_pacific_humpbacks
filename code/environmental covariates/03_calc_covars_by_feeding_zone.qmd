...
First, for sea surface temperature, June to August mean.
...
Chlorophyll, April/May mean.

```{r}
chl_ts <- map(fzones$zoneID,function(zone) {
  calc_ts(v="chl",year_start=1993,year_end=2024,month_start=4,month_end=5,z=zone)
}) |> bind_rows()
...
Mixed layer depth, June to August mean.
...
Nitrate, April/May mean

```{r}
no3_ts <- map(fzones$zoneID,function(zone) {
  calc_ts(v="no3",year_start=1993,year_end=2024,month_start=4,month_end=5,z=zone)
}) |> bind_rows()
...
Net primary production, April/May.

```{r}
nppv_ts <- map(fzones$zoneID,function(zone) {
  calc_ts(v="nppv",year_start=1993,year_end=2024,month_start=4,month_end=5,z=zone)
}) |> bind_rows()
...
Produce time series like this for all variables and for all feeding zones. Now using a baseline of 1993-2013. Temperature and MLD will be averaged over the summer months (when the whales are feeding), while we measure chlorophyll, NO3, and NPPV in April/May to capture the spring bloom dynamics.
...
chl_anoms <- map(fzones$zoneID,function(zone) {
  calc_anom_ts(v="chl",year_start=1993,year_end=2024,baseline_start = 1993,baseline_end = 2013,month_start=4,month_end=5,z=zone,baseline_fun='mean') %>% mutate(zoneID=zone)
}) %>% bind_rows()
...
no3_anoms <- map(fzones$zoneID,function(zone) {
  calc_anom_ts(v="no3",year_start=1993,year_end=2024,baseline_start = 1993,baseline_end = 2013,month_start=4,month_end=5,z=zone,baseline_fun='mean') %>% mutate(zoneID=zone)
}) %>% bind_rows()
...
nppv_anoms <- map(fzones$zoneID,function(zone) {
  calc_anom_ts(v="nppv",year_start=1993,year_end=2024,baseline_start = 1993,baseline_end = 2013,month_start=4,month_end=5,z=zone,baseline_fun='mean') %>% mutate(zoneID=zone)
}) %>% bind_rows()
...
Plot the final time series that will be used to drive the models

```{r}
anommat <- map2(out,c('sst','chl','mld','no3','nppv'),\(x,y) x |> mutate(variable=y)) |> 
  bind_rows() |> 
  dplyr::select(-sd) |> 
  group_by(zoneID,variable) |>
  mutate(anom_norm=(anom-mean(anom))/sd(anom)) |> 
  ungroup()

# Time series
panom_ts <- anommat |> 
  filter(zoneID %in%c("EAL+BER_16","GOA_18","OR+CA_13","RUS+WAL_15","SBC+WA_11","SEA+NBC_12")) |> 
  ggplot(aes(year,anom_norm,color=variable))+
  geom_line()+
  facet_grid(variable~zoneID)+
  geom_hline(yintercept=0,linetype=2)+
  scale_color_manual(values=viridis_pal(option="turbo")(5))+
  labs(y="Anomaly",x="Year")+
  theme_minimal()+theme(panel.border=element_rect(color='black',fill=NA))
panom_ts
ggsave(here('plots','final','environmental covariates','anomaly timeseries.png'))
```



How closely correlated are all these variables?
...
anomwide <- anommat |> 
  select(-anom) |> 
  pivot_wider(names_from="variable",values_from="anom_norm")
...
anomcor <- map(anomzl,\(x) x |> select(sst:nppv) |> cor())
names(anomcor) <- zn$zoneID

# Save anomaly correlation plots for feeding grounds in this study
whichp <- which(names(anomcor)%in% c("EAL+BER_16","GOA_18","OR+CA_13","RUS+WAL_15","SBC+WA_11","SEA+NBC_12"))

walk(whichp,\(x){
  corm <- anomcor[[x]]
  nm <- names(anomcor)[x]
  png(here('plots','final','environmental covariates',paste(nm,"corr.png")), width = 1000, height = 1000, res = 200) 
  
  corrplot(corm, method = "ellipse",type='upper',addCoef.col='black',title=nm,mar=c(0,0,2,0))
