...
# ========================================================================================================================
build_envS_rvec_parameters <- function(base_params, epsEnv, log_sigmaEnv, envParams, Nfeed) {
  #' Add environment-driven survival parameters with feeding-ground-specific rval
  #'
  #' The envS-rvec model:
  #'   - Drives survival DIRECTLY with environmental covariates
  #'   - Estimates SEPARATE intrinsic growth rate (rval) for each feeding ground
  ...
  #'   - rval: Vector of intrinsic growth rates (one per feeding ground)
  ...
  # Override rval to be feeding-ground-specific
  params$rval <- rep(0.09, Nfeed)
...
# BUILD RTMB MAP
# ========================================================================================================================
build_envS_rvec_map <- function(base_map, AddCV = FALSE) {
  #' Build the RTMB map constraints for envS-rvec model
  #'
  #' The envS-rvec model estimates:
  #'   - rval: Vector of growth rates (ESTIMATED - one per feeding ground)
  #'   - epsEnv: Environmental random effect (treated as random, not in map)
  #'   - log_sigmaEnv: Variance of environmental effect (ESTIMATED)
  #'   - envParams: Environmental coefficients (ESTIMATED)
...
f <- function(parms,dat)
 {
  ...
  # Equilibrium fecundity calculation (per feeding ground) - rval is now a VECTOR
  fmax <- rep(0, Nfeed)
  f0 <- rep(0, Nfeed)
  ParA <- rep(0, Nfeed)
  
  for (Ifeed in 1:Nfeed) {
    fmax[Ifeed] <- 2*(exp(rval[Ifeed]*(Amat+1.0))-SA*exp(rval[Ifeed]*Amat))/(SC*SA^(Amat))
    f0[Ifeed] <- 2*(1.0-SA)/(SC*SA^(Amat))
    ParA[Ifeed] <- (fmax[Ifeed]-f0[Ifeed])/f0[Ifeed]
  }
    ...
    for (Ibreed in 1:Nbreed){
      for (Ifeed in 1:Nfeed){
        if (MixI[Ibreed,Ifeed] != 0){
          ...
          SurvSCF <- 1.0/(1.0+exp(LogitSC+SFdevYr[Ifeed,Year]));
          SurvJuv <-  SurvSCF
          
          # Fecundity (Density-dependent)
          Depl <- Nf[Ifeed,Year]/(FeedK[Ifeed]*MultK[Ifeed,Year]);
          Term1 <- f0[Ifeed]*(1.0+ParAV[Ifeed,Year]*(1.0-Depl));
  ...
  # Penalties/Priors
  # =================================================================================================================================
  # penalty on fmax (applied to all feeding grounds)
  Penal_fmax <- sum(100.0/(1+exp(-1000.0*(fmax-0.98))))
  
  # Prior on rval (weak regularization toward biological reasonableness)
  rval_prior <- -sum(dnorm(rval, mean=0.09, sd=0.05, log=TRUE))
  
  Penal <- Penal_fmax + rval_prior
