...
ReadSurveyData <- function(Code,BreedingOpt,FeedingOpt,BreedNames,FeedNames,Yr1,Yr2,SensTest="")
 {
  ...
  Surveys <- read.csv("Inputs/SurveyAll.duringWorkshop1.csv",fill=T,comment.char="?",header=T,row.names=NULL)[,1:15]
  colnames <- c("Year1","Year2","UseVarCo","VarCo","Estimate","CV","Area","Rel","Use","Add.cv","Hypothesis","Class","SensUse","Component","Reference")
  ...
  NsurveySeries <- max(unique(SurveyI[,5]+1))
  SurveySeries <- c(1,rep(2,NsurveySeries-2),1)
  
  VarCoUse <- rep(0,NsurveySeries)
  for (Iseries in 1:NsurveySeries) if (any(Surveys$UseVarCo[which(Surveys$Class==Iseries)]=="Yes")) VarCoUse[Iseries] <- 1

  VarCo <- read.csv("Inputs/VarCo.csv",fill=T,comment.char="?",header=F,row.names=NULL)

  VarCoMat <- array(0,dim=c(Nvarco-1,1000,1000))
  Ipnt <- 1
  for (Ivarco in 1:(Nvarco-1))
   {
