...
f <- function(parms)
 {
  ...
  for (Iclass in 1:NsurveySeries)
   {
    if (SurveySeries[Iclass]==1 & VarCoUse[Iclass]==1)
     {
      Nresids <- length(which(SurveyI[,5]==Iclass))
      Resids <- rep(0,Nresids); Adds <- rep(0,Nresids); UseDataPnt <- F; Ipnt <- 0
      for (Isurv in 1:NsurveyData)
       if (SurveyI[Isurv,5] == Iclass)
        {
         if (SurveyI[Isurv,6]==1) UseDataPnt <- T 
         Ipnt <- Ipnt + 1
         Pred <- 0;
         ...
         PredSurv[Isurv] <- Pred;
         print(c(1,Isurv,SurveyR[Isurv,1],PredSurv[Isurv]))
         Resids[Ipnt] <- log(SurveyR[Isurv,1])-log(PredSurv[Isurv])
         if (SurveyI[Isurv,7]>0) Adds[Ipnt] <-AddV[SurveyI[Isurv,7]]
      ...
      for (II in 1:Ipnt)
       {
        for (JJ in 1:Ipnt) VarCo1[II,JJ] <- VarCoMat[1,II,JJ]
      ...
      if (UseDataPnt==T) LogLike1 <- LogLike1 - dmvnorm(x=Resids,Sigma=VarCo2,log=T)
  
     } # Absolute indicies

    if (SurveySeries[Iclass]==1 & VarCoUse[Iclass]==0)
     {
      for (Isurv in 1:NsurveyData)
        if (SurveyI[Isurv,5] == Iclass)
        {
          ...
          SurveySD[Isurv] <- sqrt(SD2);
          LikeCompSurv[Isurv] <- -dnorm(log(SurveyR[Isurv,1]), log(PredSurv[Isurv]), sqrt(SD2), log=T);
          print(c(2,Isurv,SurveyR[Isurv,1],PredSurv[Isurv],LikeCompSurv[Isurv]))
          if (SurveyI[Isurv,3]==1) LikeCompSurv[Isurv] <- LikeCompSurv[Isurv] * WghtTotal;
                  ...
                  BootUse,SetNew=0,Init=NULL,Nage=11,DoBayes=F)
{
  ...
  PlotIndex(cbind(SurveyI,SurveyR),Nt.est,Nt.see,Nb.est,Nb.see,Nf.est,Nf.see,rept$Qest,PDF=PDF,Code=Code2,BreedNames,FeedNames,Nyear,Years,Yr1,Yr2,Nbreed,Nfeed,rept,AllPlots=AllPlots)
