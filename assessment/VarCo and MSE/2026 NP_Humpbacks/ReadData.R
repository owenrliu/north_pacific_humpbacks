ReadCatches <- function(DatFile,BreedingOpt,FeedingOpt,Nbreed,Nfeed,BreedNames,FeedNames,Yr1,Yr2,CatchSer,ByCatchFile="Null.csv")
 {
  ...
  CatchCom <- read.csv("inputs\\CatchCom.csv",skip=1,head=F)[,-1]
  CatchBy <- read.csv(paste0("inputs\\",ByCatchFile),skip=1,head=F)[,-1]
...
ReadSurveyData <- function(Code,BreedingOpt,FeedingOpt,BreedNames,FeedNames,Yr1,Yr2,SensTest="")
 {
  ...
  Surveys <- read.csv("Inputs/SurveyAll.duringWorkshop2_rev.csv",fill=T,comment.char="?",header=T,row.names=NULL)[,1:14]
  colnames <- c("Year1","Year2","VarCo","Estimate","CV","Area","Rel","Use","Add.cv","Hypothesis","Index","SensUse","Component","Reference")
  ...
  write.csv(Surveys,file=paste0("Diags/SurveyUse",Code,SensTest,".csv"),row.names=F)
  Surveys$VarCo <- as.numeric(Surveys$VarCo); Nvarco <- max(Surveys$VarCo)
  ...
  NextraCV1 <- 0
  SurveyI<- NULL; SurveyR <- NULL; N.SurveySeries <- max(Surveys$"Index")
  SurveySeries <- rep(-1,N.SurveySeries)
    ...
    if (Type != -1)
     {
      ...
      Isurv <- Surveys$Index[Iline]; if (Isurv > 0 & Surveys$Rel[Iline] == "Abs") SurveySeries[Isurv] <- 1  
      Isurv <- Surveys$Index[Iline]; if (Isurv > 0 & Surveys$Rel[Iline] == "Rel") SurveySeries[Isurv] <- 2 
      Vec1 <- c(Surveys$Year1[Iline]-Yr1,Surveys$Year2[Iline]-Yr1,Type,Area,Isurv,Surveys$Add.cv[Iline],Surveys$Component[Iline])
  ...
  colnames(SurveyI) <- c("Year1","Year2","Type","Area","Index","Add.cv","Component")
  colnames(SurveyR) <- c("Estimate","Sd_log")

  NsurveyData <- length(SurveyI[,1])
  # Number of survey series
  NsurveySeries <- length(SurveySeries)
  VarCoUse <- rep(0,NsurveySeries)
  for (Iseries in 1:NsurveySeries) if (any(Surveys$VarCo[Surveys$Index==Iseries]>0)) VarCoUse[Iseries] <- Surveys$VarCo[Surveys$Index==Iseries][1]

  VarCo <- read.csv("Inputs/VarCo.csv",fill=T,comment.char="?",header=F,row.names=NULL)
  VarCoMat <- array(0,dim=c(Nvarco,1000,1000))
  Ipnt <- 0
  for (Ivarco in 1:Nvarco)
   {
    Nlines <- sum(Surveys$VarCo==Ivarco)
    # Now read in the 
    Ipnt <- Ipnt + 1
    for (Irow in 1:Nlines)
     {
      Ipnt <- Ipnt + 1
      for (Jrow in 1:Nlines)  VarCoMat[Ivarco,Irow,Jrow] <- as.numeric(VarCo[Ipnt,1+Jrow]) 
     } # Irow
   } # Ivarco
  ...
  Outs$VarCoMat <- VarCoMat
  Outs$VarCoUse <- VarCoUse
  aa <<- Outs
  return(Outs)
 }
...
ReadMixingData <- function(Code,DatFile,Yr1,Yr2,Nbreed,Nfeed,BreedNames,FeedNames,MixWeights,SensTest="",MaxN=-1000)
{
  ...
  # Read in mixing data
  # =================== 
  MixFile1 <- read.csv("Inputs/Genetics_mixing_data_allscenarios_table_Long.csv")
  MixFile2 <- read.csv("Inputs/Mark-Recapture_mixing_data_allscenarios_table_Long.csv")
  ...
  FileName <- paste0("Diags/MixUse",Code,SensTest,".csv")
