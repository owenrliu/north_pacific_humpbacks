...
setwd("C:\\Research\\Iwc27\\NP_Humpbacks\\")
source("HumpFigs2.R")
source("ReadData.R")
source("WriteOut.R")
...
ProjectV <- function(parms)
{
  ...
  getAll(data2, parms, warn=FALSE)                                  # Make the data and parameters global to this function
  NyrHist <- Yr2-Yr1+1;
  Nyr <- Yr2-Yr1+1+Nproj;                                           # Number of years (including projection)
  MixPar <- length(MixPars);                                        # Number of mixing parameters
  ...
  if (Projection==T)
   {
    ...
    data2$Nproj <<- 0
    data2$DoLike <- F
    model2 <- MakeADFun(ProjectV, parameters, map=map,silent=T)
    print("doine")
    data2$Nproj <<- 10
    rept2 <<- model2$report()
    print(str(rept2))
...
if (Case==1)
 {
  ...
  xx <- DoRun("B2F1",SensCase="BC",AllPlots=T,DoBoot=F,Init=NULL,SetNew=0)
