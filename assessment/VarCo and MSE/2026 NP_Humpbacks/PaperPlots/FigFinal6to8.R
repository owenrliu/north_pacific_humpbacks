...
ReadData <- function(Code,Code2,Yr1=1965,Yr2=2023,Nboot,type="a",PDF=F)
{
  ...
  SDsT <- rep(0,Nyear)
  Index1 <- Index[1]
  for (Iyr in 1:Nyear) SDsT[Iyr] <- as.numeric(DatFile[Index1+Iyr,5]) 
  for (II in 0:Nboot)
   {
    Index1 <- Index[II+1]
    for (Iyr in 1:Nyear)
      SummaryT[Iyr,II+1] <- as.numeric(DatFile[Index1+Nyear+1+Iyr,4]) 
    Index1 <- IndexS[II+1]
...
Resu[[2]] <- ReadData("B1F2","BC",Yr1=1970,Yr2=2023,Nboot=500,PDF=T)
Resu[[3]] <- ReadData("B2F1","BC",Yr1=1970,Yr2=2023,Nboot=500,PDF=T)
Resu[[4]] <- ReadData("B2F2","BC",Yr1=1970,Yr2=2023,Nboot=500,PDF=T)

png(filename="E:/Fig6a1.png",width=700,height=800)
par(mfcol=c(2,2),oma=c(3,4,3,4),mar=c(4,4,2,2))
Fig1a(Resu[[1]],type="B1-F1")
Fig1a(Resu[[2]],type="B1-F2")
mtext("Abundance",side=3,line=1)
mtext("Year",side=1,line=1)
dev.off()
png(filename="E:/Fig6a2.png",width=700,height=800)
par(mfcol=c(2,2),oma=c(3,4,3,4),mar=c(4,4,2,2))
Fig1a(Resu[[1]],type="B2-F1")
Fig1a(Resu[[2]],type="B2-F2")
mtext("Abundance",side=3,line=1)
mtext("Year",side=1,line=1)
dev.off()
