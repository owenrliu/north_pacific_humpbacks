...
PlotPropns <- function(Nprop,ObsMixPropI,ObsMixProp,PredMix,PDF=T,Code="No model",
                       BreedNames,FeedNames,Nyear)
{
  ...
  box();  axis(2)
  stars <- NULL
  for (II in 1:NpropV) if (VecOut[II,3]==0) stars <- c(stars,"*") else stars <- c(stars,"")
  fonts <- NULL
  for (II in 1:NpropV) if (VecOut[II,3]==0) fonts <- c(fonts,3) else fonts <- c(fonts,1)
  XLAB <- NULL
  for (II in 1:NpropV)
    XLAB <- c(XLAB,paste(BreedNames[VecOut[II,1]],"-",FeedNames[VecOut[II,2]],stars[II]))  
  for (II in 1:NpropV) 
  ...
  for (II in 1:NpropV)  axis(1,II,XLAB[II],las=2,cex.axis=1,font.axis=fonts[II])
...
PlotIndex <- function(SurveyD,Nt.est,Nt.see,Nb.est,Nb.see,Nf.est,Nf.see,Qest,PDF=T,Code="No model",
                      BreedNames,FeedNames,Nyear,Years,Yr1,Yr2,Nbreed,Nfeed,rept,AllPlots,
                      SurveySeries)
 {
  ...
  SD1 <- Index2[,9];
  SDD <- SD1
  if (Index2[1,6]>0) SDD <- sqrt(SD1^2+rept$AddV[Index2[1,6]])
  ...
  for (II in 1:length(Index2[,1]))
  {
    ...
    if (Index2[1,6]==0) lines(c(Yr,Yr),c(Low1[II],Upp1[II]),lty=1,lwd=1)
    if (Index2[1,6]>0) lines(c(Yr,Yr),c(Low1[II],Upp1[II]),lty=1,lwd=2,col="red",lty-1)
    if (Index2[1,6]>0) lines(c(Yr,Yr),c(Low2[II],Upp2[II]),lty=1,lwd=1)
  }  
  title("All")
  ...
  Series <- which(SurveySeries==1)
  for (Ibreed in 1:Nbreed)
   {
     Index2 <- SurveyD[SurveyD[,3]== 2 & SurveyD[,4]== Ibreed & SurveyD[,5] %in% Series,]
    ...
    for (II in 1:length(Index2[,1]))
    {
      ...
      points(Yr,Index2[II,8],pch=16)  
      if (Index2[1,6]==0) lines(c(Yr,Yr),c(Low1[II],Upp1[II]),lty=1,lwd=1)
      if (Index2[1,6]>0) lines(c(Yr,Yr),c(Low1[II],Upp1[II]),lty=1,lwd=2,col="red")
      if (Index2[1,6]>0) lines(c(Yr,Yr),c(Low2[II],Upp2[II]),lty=1,lwd=1)
    }  
  ...
  {
    Index2 <- SurveyD[SurveyD[,3]== 3 & SurveyD[,4]== Ifeed & SurveyD[,5] %in% Series,]
    if (length(Index2)==9) Index2 <- t(Index2)
    Icomp <- Index2[1,7]+1

    SD1 <- Index2[,9]
    SDD <- SD1
    if (Index2[1,6]>0) SDD <- sqrt(SD1^2+rept$AddV[Index2[1,7]])
    Low1 <- Index2[,8]*exp(-1.96*SD1)
    Upp1 <- Index2[,8]*exp(1.96*SD1)
    Low2 <- Index2[,8]*exp(-1.96*SDD)
    Upp2 <- Index2[,8]*exp(1.96*SDD)
  ...
  for (Iseries in 1:length(SurveySeries))
   {
    Index2 <- SurveyD[SurveyD[,5]== Iseries,]
    if (length(Index2)>0 & SurveySeries[Iseries]==2)
     {  
      if (length(Index2)==9) Index2 <- t(Index2)
      if (Index2[1,3]==2)
       {
